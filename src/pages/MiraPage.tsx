  ...
  return (
    <div className="min-h-screen bg-gray-950 flex flex-col">
      <div className="border-b border-gray-800 bg-gray-900 px-4 py-3 flex items-center gap-3">
        <div className="w-9 h-9 rounded-full bg-violet-600 flex items-center justify-center text-white font-bold text-sm">M</div>
        <div><p className="font-semibold text-white text-sm">Mira AI</p><p className="text-xs text-gray-400">Educational assistant · Not medical advice</p></div>
      </div>
      ...
      <p className="text-xs text-gray-600 text-center px-4 pb-1 max-w-2xl mx-auto w-full">{DISCLAIMER}</p>
      <div className="border-t border-gray-800 bg-gray-900 px-4 py-3 max-w-2xl mx-auto w-full">
        <div className="flex gap-2">
          <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&!e.shiftKey&&send()} placeholder="Ask Mira a question…" className="flex-1 rounded-xl border border-gray-700 bg-gray-950 px-4 py-2.5 text-sm text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-violet-500"/>
