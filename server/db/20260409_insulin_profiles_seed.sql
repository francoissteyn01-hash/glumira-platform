-- 1. Create TYPE if not exists (optional, but explicit)
DO $$ BEGIN
    ...
    CREATE TYPE decay_model AS ENUM ('exponential', 'bilinear', 'flat_depot', 'mixed_profile');
EXCEPTION
...
CREATE TABLE IF NOT EXISTS insulin_profiles (
    ...
    decay_model decay_model NOT NULL,
...
-- 4. Seed data (shortened strings to avoid truncation)
...
('Apidra', 'insulin glulisine', 'Sanofi', 'rapid_acting', 15, 30, 90, 340, FALSE, 'Glulisine for fast onset', 'Apidra Prescribing Information 2023', 'exponential', '{"half_life_minutes": 80}'),
('Basaglar', 'insulin glargine', 'Eli Lilly', 'long_acting', 60, NULL, NULL, 1440, FALSE, 'Steady glargine release', 'Basaglar Prescribing Information 2023', 'bilinear', '{"slope_1_pct_per_min": 0.0005, "inflection_minutes": 320, "slope_2_pct_per_min": 0.0001}'),
('Fiasp', 'faster insulin aspart', 'Novo Nordisk', 'rapid_acting', 3, 26, 35, 300, FALSE, 'Faster aspart via additives', 'Fiasp Prescribing Information 2023', 'exponential', '{"half_life_minutes": 75}'),
('Humalog', 'insulin lispro', 'Eli Lilly', 'rapid_acting', 15, 30, 90, 360, FALSE, 'Lispro monomer swap', 'Humalog Prescribing Information 2023', 'exponential', '{"half_life_minutes": 90}'),
('Humalog Mix 25', 'insulin lispro / lispro protamine', 'Eli Lilly', 'mixed', 15, 60, 180, 1440, FALSE, 'Rapid + intermediate mix', 'Humalog Mix Prescribing Information 2023', 'mixed_profile', '{"rapid_pct": 0.25, "rapid_params": {"half_life_minutes": 90}, "basal_pct": 0.75, "basal_params": {"slope_1_pct_per_min": 0.0007, "inflection_minutes": 360, "slope_2_pct_per_min": 0.0002}}'),
...
('Levemir', 'insulin detemir', 'Novo Nordisk', 'long_acting', 60, 360, 520, 1440, FALSE, 'Detemir fatty acid binding', 'Levemir Prescribing Information 2023', 'bilinear', '{"slope_1_pct_per_min": 0.0007, "inflection_minutes": 440, "slope_2_pct_per_min": 0.0002}'),
...
('Toujeo', 'insulin glargine U-300', 'Sanofi', 'long_acting', 360, NULL, NULL, 2160, FALSE, 'U-300 flatter profile', 'Toujeo Prescribing Information 2023', 'bilinear', '{"slope_1_pct_per_min": 0.0003, "inflection_minutes": 720, "slope_2_pct_per_min": 0.00005}'),
('Tresiba', 'insulin degludec', 'Novo Nordisk', 'ultra_long_acting', 60, NULL, NULL, 2520, TRUE, 'Flat depot absorption', 'Heise et al. 2012, Diabetes Obes Metab; Tresiba FDA Prescribing Information 2023', 'flat_depot', '{"steady_rate_pct_per_hour": 2.38}') ON CONFLICT (brand_name) DO NOTHING;
